...
Copyright(C) 2006, Commtech, Inc.
  ...
  trx.c -- a user mode program to read bytes from a channel and stuff them to a file
	   -- configures the port to transparent mode using sync signals (FSR) to gate the receive (mode taken from the command line)
 ...
 usage:
  read port fsyncmode outfile [display]

  (do not use display except for debugging, as it will likely make gaps in the data)
  tested fsyncmodes are 1 (fsync active for entire frame) and 2 (fsync active for first bit)
  mode 3 likely won't work
  mode 4 (both 2&3) might work
...
int main(int argc, char *argv[])
{
	...
	{
		printf("usage:\n");
		printf("read port synctype outfile\n");
		exit(1);
	}
	if(argc>4) display=1;
	else display=0;
...
myset.ccr0         = 0x00100006 | ((synctype&7)<<8);//X-Sync,2 sync bytes, no term bytes,clock mode 1,LSB, NRZ, recevier ON, idle '1's
