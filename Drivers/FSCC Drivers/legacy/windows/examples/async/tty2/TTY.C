...
if(BAUDRATE(npTTYInfo)==userrate)
{
char buf[256];
BAUDRATE(npTTYInfo)=0xdeadbeef;
sprintf(buf,"%d",userrate);
SendDlgItemMessage(hDlg,IDC_USER_RATE,WM_SETTEXT,(WPARAM)0,(LPARAM)buf);
}
...
BOOL NEAR SettingsDlgTerm( HWND hDlg )
{
   ...
   if(BAUDRATE(npTTYInfo)==0xdeadbeef)
	{
	char buf[256];
	SendDlgItemMessage(hDlg,IDC_USER_RATE,WM_GETTEXT,(WPARAM)sizeof(buf),(LPARAM)buf);
	BAUDRATE(npTTYInfo) = atol(buf);
	userrate = atol(buf);
	}
#else
   BAUDRATE( npTTYInfo ) =
